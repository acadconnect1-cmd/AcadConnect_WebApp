...
export function EditJobClient({ job, slug }: EditJobClientProps) {
  const router = useRouter()
  const [isSubmitting, setIsSubmitting] = useState(false)
  const [message, setMessage] = useState<{ type: 'success' | 'error'; text: string } | null>(null)

  // Format deadline date string for input default value
  const defaultDeadline = job.application_deadline
  ...
  const {
    ...
    defaultValues: {
      ...
      requiredQualification: job.required_qualification,
      preferredQualification: job.preferred_qualification || '',
      applicationDeadline: defaultDeadline,
      status: job.status,
    },
  })

  const handleUpdate = async (status: 'draft' | 'published' | 'closed' | 'archived', data: JobFormInput) => {
    ...
    const res = await updateJobAction(job.id, formData as any)
    setIsSubmitting(false)

    if (res.success) {
      setMessage({ type: 'success', text: `Vacancy listing successfully updated as ${status}!` })
      setTimeout(() => {
      ...
      <div className="space-y-4">
        ...
        <div>
          ...
          <p className="text-muted-foreground text-sm mt-1">
            Update your posting specifications and settings.
          </p>
        </div>
      </div>
        ...
        <div className="flex flex-col sm:flex-row justify-end items-center gap-3 border-t border-border/60 pt-6">
          ...
          {job.status === 'published' ? (
            <Button
              type="button"
              disabled={isSubmitting}
              onClick={handleSubmit((data) => handleUpdate('closed', data))}
              className="w-full sm:w-auto h-11 px-6 bg-amber-600 hover:bg-amber-700 text-white font-bold rounded-xl gap-1.5 shadow-md text-xs"
            >
            ...
            <Button
              type="button"
              disabled={isSubmitting}
              onClick={handleSubmit((data) => handleUpdate('published', data))}
            ...
            >
              {isSubmitting ? <Loader2 className="h-4 w-4 animate-spin" /> : 'Publish Position'}
            </Button>
          )}
        </div>
      </form>
